...
const LETTERS_KEY = 'cc_last_letter';
...
function LettersSection({
  ...
  // Restore last generated letter on mount
  useEffect(() => {
    AsyncStorage.getItem(LETTERS_KEY).then(raw => {
      if (!raw) return;
      const saved = JSON.parse(raw) as { company: string; role: string; letterType: LetterType; letter: string };
      if (saved?.letter) {
  ...
  async function generateLetter() {
    ...
    try {
      ...
      AsyncStorage.setItem(LETTERS_KEY, JSON.stringify({
        company: company.trim(), role: role.trim(), letterType, letter: data.letter,
      })).catch(() => {});
...
function InterviewSection({
  ...
  async function finishInterview(finalAnswers: string[]) {
    ...
    try {
      const data = await aiService.interviewVerdict({
      ...
      setStage('verdict');
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      // Persist verdict to matching tracked application
      const matchingApp = applications.find(
        a => a.companyName.toLowerCase() === selected.name.toLowerCase()
      );
      if (matchingApp) {
        updateApplication(matchingApp.id, { interviewVerdict: JSON.stringify(data) });
      }
    } catch {
...
const styles = (colors: ReturnType<typeof useColors>) => StyleSheet.create({
  ...
  formCard: { backgroundColor: colors.card, borderRadius: 22, padding: 22, borderWidth: 1, borderColor: colors.border, marginBottom: 20, gap: 8 },
  formLabel: { fontSize: 13, fontFamily: 'Inter_600SemiBold', color: colors.textSecondary, marginTop: 12, marginBottom: 6 },
  input: { backgroundColor: colors.background, borderRadius: 14, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 16, paddingVertical: 15, fontSize: 14, fontFamily: 'Inter_400Regular', color: colors.text },
  ...
  companyPickCard: { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: colors.card, borderRadius: 20, padding: 20, borderWidth: 1, borderColor: colors.border, marginBottom: 14 },
  ...
  briefingCard: { backgroundColor: colors.card, borderRadius: 20, padding: 22, borderWidth: 1, borderColor: colors.border, marginBottom: 20 },
  ...
  feedbackCard: { backgroundColor: colors.card, borderRadius: 20, padding: 22, borderWidth: 1, borderColor: colors.border, marginBottom: 18 },
  ...
  answerFeedbackCard: { backgroundColor: colors.card, borderRadius: 18, padding: 20, borderWidth: 1, borderColor: colors.border, marginBottom: 14 },
